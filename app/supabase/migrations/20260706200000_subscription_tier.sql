-- Tier de assinatura (Start / Pro) para planos Mercado Pago da plataforma.

ALTER TABLE public.barbershops
  ADD COLUMN IF NOT EXISTS subscription_tier text;

ALTER TABLE public.barbershops
  DROP CONSTRAINT IF EXISTS barbershops_subscription_tier_check;

ALTER TABLE public.barbershops
  ADD CONSTRAINT barbershops_subscription_tier_check
  CHECK (subscription_tier IS NULL OR subscription_tier IN ('start', 'pro'));

COMMENT ON COLUMN public.barbershops.subscription_tier IS
  'Plano ativo: start (R$39,90) ou pro (R$49,90). Pro libera cobrança no link público.';
