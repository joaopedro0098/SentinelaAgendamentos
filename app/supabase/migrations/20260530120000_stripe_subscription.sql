-- Assinatura recorrente com cartão via Stripe (Pix continua no Mercado Pago).

ALTER TABLE public.barbershops
  ADD COLUMN IF NOT EXISTS stripe_customer_id text,
  ADD COLUMN IF NOT EXISTS stripe_subscription_id text;

COMMENT ON COLUMN public.barbershops.stripe_customer_id IS
  'Cliente Stripe vinculado ao dono da barbearia.';
COMMENT ON COLUMN public.barbershops.stripe_subscription_id IS
  'Assinatura recorrente Stripe (cartão). Pix não usa este campo.';
