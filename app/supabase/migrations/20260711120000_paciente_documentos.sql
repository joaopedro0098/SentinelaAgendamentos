-- Documentos do paciente (aba Pacientes → Documentos).
...
CREATE TABLE IF NOT EXISTS public.paciente_documentos (
  ...
  CONSTRAINT paciente_documentos_size_check CHECK (size_bytes > 0 AND size_bytes <= 10485760),
  CONSTRAINT paciente_documentos_mime_check CHECK (
    mime_type IN (
      'application/msword',
...
CREATE INDEX IF NOT EXISTS paciente_documentos_whatsapp_idx
  ON public.paciente_documentos (whatsapp_digits, created_at DESC);
...
CREATE POLICY paciente_documentos_select_painel
  ...
  USING (
    barbearia_id = ANY(public.painel_barbearia_ids_pacientes_visiveis())
  );

CREATE POLICY paciente_documentos_delete_painel
  ON public.paciente_documentos FOR DELETE TO authenticated
  USING (
    uploaded_by = auth.uid()
    AND barbearia_id = ANY(public.painel_barbearia_ids_editaveis())
  );

COMMENT ON TABLE public.paciente_documentos IS
  'Metadados de documentos anexados ao paciente (storage privado paciente-documentos).';

-- Bucket privado com limite de 10 MB e MIME types permitidos.
...
DROP POLICY IF EXISTS paciente_documentos_select_own ON storage.objects;
DROP POLICY IF EXISTS paciente_documentos_insert_own ON storage.objects;
DROP POLICY IF EXISTS paciente_documentos_update_own ON storage.objects;
...
CREATE POLICY paciente_documentos_update_own
...
AS $$
  SELECT
    ...
    AND EXISTS (
      ...
      WHERE a.barbearia_id = ANY(public.painel_barbearia_ids_pacientes_visiveis())
...
AS $$
  SELECT
    ...
    AND EXISTS (
      SELECT 1
      FROM public.agendamentos a
      WHERE a.barbearia_id = ANY(public.painel_barbearia_ids_editaveis())
        AND public.cliente_whatsapp_digits(a.cliente_whatsapp) = public.cliente_whatsapp_digits(p_whatsapp_digits)
        AND (
          a.status = 'concluido'::public.agendamento_status
          OR EXISTS (
...
BEGIN
  ...
  FROM (
    ...
    WHERE pd.whatsapp_digits = _digits
      AND pd.barbearia_id = ANY(public.painel_barbearia_ids_pacientes_visiveis())
  ) d;
...
BEGIN
  ...
  WHERE pd.id = p_documento_id
    ...
    AND pd.barbearia_id = ANY(public.painel_barbearia_ids_editaveis());
