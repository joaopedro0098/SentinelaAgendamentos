-- Titular AA (is_admin_aggregated) deve contar como elegível a pagamentos,
-- assim como no get_my_subscription. Sem isso, CAs de CT AA viam mensagem de Start.

CREATE OR REPLACE FUNCTION public.shop_can_use_appointment_payments(p_shop public.barbershops)
RETURNS boolean
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT coalesce(p_shop.is_admin_aggregated, false)
    OR (
      p_shop.subscription_tier = 'pro'
      AND p_shop.subscription_status IN ('active', 'grace', 'cancelled')
      AND (
        p_shop.current_period_end IS NULL
        OR p_shop.current_period_end >= CURRENT_DATE
      )
    );
$$;

COMMENT ON FUNCTION public.shop_can_use_appointment_payments(public.barbershops) IS
  'True se shop é AA (admin) ou Pro ativo. Usado para painel Pagamentos de CA e cobrança no link.';

-- get_my_subscription: CA herda pagamentos do titular AA
