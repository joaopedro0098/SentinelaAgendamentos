-- Painel: herda subscription mais recente do cliente (link público primeiro), não a mais antiga com last_success_at.
...
BEGIN
  ...
  WHERE a.barbearia_id = _barbearia_id
    ...
    AND s.failed_at IS NULL
  ORDER BY
    CASE WHEN a.origem = 'link_publico' THEN 0 ELSE 1 END,
    s.created_at DESC
  LIMIT 1;
