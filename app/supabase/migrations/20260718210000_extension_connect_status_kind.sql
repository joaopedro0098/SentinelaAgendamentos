-- Connect: status futuro alinhado à aba Agendamentos (Confirmado / Não confirmado).
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN a.status = 'aguardando_pagamento'::public.agendamento_status THEN 'aguardando_pagamento'
        WHEN coalesce(a.requires_client_confirmation, false) AND a.client_confirmed_at IS NULL THEN 'nao_confirmado'
        ELSE 'confirmado'
      END AS status_kind
    FROM public.agendamentos a
...
COMMENT ON FUNCTION public.extension_connect_client_lookup(uuid, text, text) IS
  'Painel Connect: status futuro igual aba Agendamentos (status_kind).';
