-- Remove ferias_vigentes do payload de get_bloqueios_painel (substituído por ferias_programadas; não usado no frontend).
...
COMMENT ON FUNCTION public.get_bloqueios_painel(uuid, date, date) IS
  'Painel Bloqueios: profissionais, bloqueios no período e férias programadas (próprias + CA no hub CT).';
