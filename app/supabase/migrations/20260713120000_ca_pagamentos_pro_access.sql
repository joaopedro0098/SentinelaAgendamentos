-- CA agregada a CT Pro: acesso completo à aba Pagamentos (pendências + regras de cobrança).
...
AS $$
  SELECT p_shop.subscription_tier = 'pro'
...
BEGIN
  ...
  IF _is_ca THEN
    ...
    IF _centralized THEN
      ...
      _config_shop := _titular_shop;
      _mp_shop := _titular_shop;
      _mp_managed_by_titular := true;
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.shop_can_use_appointment_payments(public.barbershops) TO authenticated, service_role;
