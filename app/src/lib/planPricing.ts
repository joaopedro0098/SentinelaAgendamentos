export const PLAN_MONTHLY_AMOUNT = 29.9;
export const PLAN_PRICE_LABEL = "R$ 29,90/mês";
export const PLAN_PRICE_SHORT = "R$ 29,90";
export const PLAN_PRICE_DISPLAY = "29,90";

export const PLAN_FEATURES = [
  "Cadastre colaboradores (ilimitados) e serviços",
  "Painel para gerenciar seus agendamentos",
  "Cliente recebe confirmação automática 1 dia antes",
  "Link para o cliente agendar por conta (opcional)",
  "Suporte conosco via WhatsApp",
  "Não exigimos cartão para fazer seu teste grátis",
];

export const PLAN_ACCOUNT_FEATURES = PLAN_FEATURES.filter(
  (feature) =>
    feature !== "Painel para gerenciar seus agendamentos" &&
    feature !== "Não exigimos cartão para fazer seu teste grátis",
);
