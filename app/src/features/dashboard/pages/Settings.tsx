...
export default function Settings() {
  ...
  useEffect(() => { document.title = "Barbearia — BarberChat"; }, []);
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { error: upErr } = await supabase.storage.from("barbershop-avatars").upload(path, file, { upsert: true });
    if (upErr) {
      setUploading(false);
  ...
  if (!shop) return <div className="p-6 text-sm text-muted-foreground">Nenhuma barbearia. Faça logout e cadastre-se novamente.</div>;
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-6">
      <header>
        <h1 className="text-2xl font-semibold">Sua barbearia</h1>
        <p className="text-sm text-muted-foreground">Personalize como o cliente vai te ver.</p>
      </header>

      {/* Perfil e link do cliente */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Perfil da barbearia</CardTitle>
